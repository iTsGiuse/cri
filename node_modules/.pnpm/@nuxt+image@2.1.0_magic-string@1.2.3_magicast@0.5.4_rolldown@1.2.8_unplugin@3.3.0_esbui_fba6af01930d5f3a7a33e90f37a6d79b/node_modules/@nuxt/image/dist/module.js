...
function isBuiltInProvider(provider2) {
  return provider2.provider?.startsWith(providerRuntime) ?? BuiltInProviders.includes(provider2.name);
}
const netlifySetup = (_providerOptions, moduleOptions, nuxt) => {
...
function generateImageOptions(providers, imageOptions) {
  const serializedImageOptions = Object.entries(imageOptions).filter(([key]) => key !== "provider").map(([key, value]) => `  ${key}: ${JSON.stringify(value)}`).join(",\n");
